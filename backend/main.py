...
# Initialize Cohere Client (replace with your actual API key or environment variable)
...
def save_chunk_to_qdrant(qdrant_client: QdrantClient, collection_name: str, chunks: list[str], embeddings: list[list[float]], url: str):
    ...
    for i, (chunk, embedding) in enumerate(zip(chunks, embeddings)):
        points.append(
            models.PointStruct(
                id=hash(f"{url}-{i}"), # Generate a unique ID for each chunk
                vector=embedding,
...
async def process_book_data():
    ...
    return {"message": "Book data processing initiated successfully."}
